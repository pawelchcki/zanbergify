...
#[no_mangle]
pub unsafe extern "C" fn zanbergify_load_model(model_path: *const c_char) -> *mut ZanbergifyModel {
    ...
    match RembgModel::load(Path::new(path_str), model_type) {
        Ok(model) => Box::into_raw(Box::new(ZanbergifyModel { inner: model })),
...
///
/// # Safety
/// `model` must be a valid pointer returned by `zanbergify_load_model`, or null.
#[no_mangle]
pub unsafe extern "C" fn zanbergify_free_model(model: *mut ZanbergifyModel) {
    if !model.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn zanbergify_process(
    input_path: *const c_char,
    output_path: *const c_char,
    model: *const ZanbergifyModel,
    thresh_low: u8,
    thresh_high: u8,
    clip_limit: f64,
    tile_size: u32,
